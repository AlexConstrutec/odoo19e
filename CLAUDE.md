...
| [construtec_attendance_sync_19](construtec_attendance_sync_19/CLAUDE.md) | n/a (new, receiving side of an API integration) | Built — modelo espejo (`construtec.attendance.mark.mirror`, upsert por `source_record_id`) que recibe el push de marcajes de asistencia desde Community (`construtec_face_attendance_19`, que vive solo ahí — Enterprise nunca marca asistencia), con su propio mapa Leaflet/wizard/controlador, independiente de `hr_attendance`. `depends: ["hr", "analytic", "construtec_ticket_billing_19"]` — Enterprise-only |
