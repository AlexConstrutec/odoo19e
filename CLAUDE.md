...
| [construtec_account_payment_order_19](construtec_account_payment_order_19/CLAUDE.md) | `bolson` | Built for `anticipo`/`anticipo_viaticos`/`pago_directo`; `pago_directo` reuses the same reconciliation engine. Also installed verbatim in Odoo19C (`payment_order_role` Solicitante/Procesador, synced via `/jsonrpc`) — see the module's own CLAUDE.md. **`anticipo_materiales` (Fase 2) in progress**: Pass 1 (tipo, líneas, Pagar a Jefe/Proveedor) built and verified; Orden de Compra / inventario / conciliación still pending — see the module's own CLAUDE.md |
...
| [construtec_ticket_billing_19](construtec_ticket_billing_19/CLAUDE.md) | n/a (new, receiving side of an API integration) | Built — receives a mirror of Odoo19C's `construtec_helpdesk_field_service` Tickets via `/jsonrpc`, lets the accountant link one or several to a real `account.move`, derives `billing_state` (Sin Facturar/Facturado/Cobrado) from that invoice. `depends: ["account"]` only. Also installed verbatim in `odoo19enterprise` |
...
| [construtec_face_attendance_19](construtec_face_attendance_19/CLAUDE.md) | n/a (new, home module is Odoo19C) | Built — núcleo de marcaje por reconocimiento facial, instalado verbatim en ambas ediciones desde 2026-09-08. `depends: ["hr_attendance", "mail", "analytic", "construtec_account_payment_order_19"]`. En Enterprise recibe marcajes empujados desde Community — ver `construtec_attendance_sync_19` |
| [construtec_attendance_sync_19](construtec_attendance_sync_19/CLAUDE.md) | n/a (new, receiving side of an API integration) | Built — modelo espejo create-only (`construtec.attendance.mark.mirror`) que recibe el push de marcajes desde Community, más una extensión del mapa del propio núcleo para verlos consolidados. `depends: ["construtec_face_attendance_19"]` only — Enterprise-only, no instalado en Odoo19C |

Dependency order (each depends on the ones above it): `construtec_hr_payroll_19` → `construtec_hr_employee_19` → `construtec_hr_reports_19` → `construtec_account_reports_19` (also depends on `construtec_hr_payroll_19` directly, for `account.payment.payslip_id`). `construtec_materials_sync_19`, `construtec_ticket_billing_19`, and `construtec_audit_log_19` are independent of this chain.
