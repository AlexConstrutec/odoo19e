...
- **`wizard/report_detalle_nomina.py` (`wizard.reporte.detalle.nomina`) deliberately does NOT use a fixed `COLUMNS` list** like the other 10 wizards do. It builds columns dynamically from whatever `code`/`name` actually appear across `payslip.line_ids` in the selected date range (`_compute_matrix()`), because different salary structures don't all share the same rule set, and this wizard's whole point is "show me exactly what's in these payslips' detail" rather than a fixed compliance-report layout. It also has a **preview (`preview_html`, an unsanitized Html field) shown before/alongside the Excel download** — the only wizard in this module with a viewer step; `_compute_matrix()` is the single source of truth for both the HTML table and the `.xlsx`, so don't let them drift by computing the matrix twice. If you add a 12th wizard needing a fixed, known column layout (a legal/compliance report), copy the pattern from `report_planilla_igss.py` instead — don't copy this one's dynamic-columns approach for that case.
